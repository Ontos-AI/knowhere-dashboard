...
import { Download, Activity, CreditCard, CheckCircle2 } from "lucide-react"
import { useTranslations } from 'next-intl'

import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { DatePickerWithRange } from "@/components/dashboard/date-range-picker"
import { UsageTable, UsageRecord } from "@/components/dashboard/usage-table"
import { BuyCreditsDialog } from "@/components/dashboard/buy-credits-dialog"
import { Separator } from "@/components/ui/separator"
import { api, JobResponse, ParseUsageResponse } from "@/lib/api"
...
export default function UsagePage() {
      ...
      try {
        ...
        const mappedJobs: UsageRecord[] = jobsResponse.jobs.map((job: JobResponse) => {
          ...
          return {
            ...
            status: status,
            duration: job.duration_seconds ? `${job.duration_seconds.toFixed(2)}s` : (job.result_metadata?.duration || '-'),
  ...
  const handleExportCSV = () => {
    ...
    const rows = jobs.map(item => [
      ...
      item.duration,
      item.cost,
      item.status === 'Done' ? tTable('statusDone') : item.status === 'Failed' ? tTable('statusFailed') : tTable('statusRunning'),
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {usageStats ? usageStats["request_total"].toLocaleString() : totalRequests.toLocaleString()}
            </div>
            <p className="text-xs text-muted-foreground">
              {usageStats 
                ? t('comparedToLastMonth', { value: `${usageStats["mom_growth"] >= 0 ? '+' : ''}${usageStats["mom_growth"]}%` })
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {usageStats ? usageStats["credits_used"].toLocaleString() : totalCost.toLocaleString()} pts
            </div>
            <p className="text-xs text-muted-foreground">
              {usageStats 
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center rounded-lg border bg-card p-1 text-card-foreground shadow-sm">
              <Button 
                variant="ghost" 
                size="sm" 
                className={cn("h-7 px-3 text-xs hover:bg-muted", activeRange === '1d' && "bg-black text-white hover:bg-black/90 dark:bg-white dark:text-black")}
                onClick={() => {
              ...
              <Button 
                variant="ghost" 
                size="sm" 
                className={cn("h-7 px-3 text-xs hover:bg-muted", activeRange === '7d' && "bg-black text-white hover:bg-black/90 dark:bg-white dark:text-black")}
                onClick={() => {
              ...
              <Button 
                variant="ghost" 
                size="sm" 
                className={cn("h-7 px-3 text-xs hover:bg-muted", activeRange === '30d' && "bg-black text-white hover:bg-black/90 dark:bg-white dark:text-black")}
                onClick={() => {
